describe('Просмотр вакансий', () => {
  it('Провальный просмотр вакансии c поиском', () => {
      cy.fixture('employer').then(data => {
          ...
          cy.get('.form-input--text').type(data.search_vacansy_incorrect);
  ...
  it('Провальный просмотр вакансии c фильтрами', () => {
    cy.fixture('employer').then(data => {
        cy.log('Переход на главную страницу');
        cy.visit('https://dev.profteam.su/');
        ...
        cy.log('Проверка провальной работы фильтров');
        cy.get('.vacancy-list__failed-loading').should('exist')
    });
  });
});